...
## Where the 3D city went

The 3D city was a page on the **website**, and you asked for the agents to come off the website — so it went with them. That was right: a customer landing on irongambit.com to buy a hoodie should not be shown your internal operations.

It now lives on **this** service instead, at `/city`, where it belongs. Same city, same enterable buildings, but the task text, timestamps and run counts are pulled live from `/state` every 20 seconds instead of being invented. The walking around is still ambient — that part is decoration, and the page says so.

It shows nothing until an agent has actually run. Hit **Run** on Houston Scout in `/admin`, then open `/city`.
...
## Key 5 — Your PIN (you pick it)

Not issued by anyone. Pick any **4 to 8 digit number** you'll remember — the same way you'd pick a debit card PIN.

Railway → Variables → **+ New Variable**:

| Name | `ADMIN_PIN` |
|---|---|
| Value | `4729` (or whatever you choose) |

Numbers only. No spaces, no quotes.

**Why a short PIN is safe here:** five wrong attempts freezes that IP for fifteen minutes, and you get an email when it happens. That turns a 10,000-guess brute force from seconds of work into years of it.
...
Open `https://your-app.up.railway.app/` — you should get a status page listing the crew.

> **Seeing "Cannot GET /"?** That means the server is running fine, you're just on an older build with no root route. Pull the latest code and redeploy.

**4. Open the dashboard.**

Go to `https://your-app.up.railway.app/admin`, paste your `ADMIN_PIN`, hit Unlock. The token is kept in that browser tab only.

From there you can:
- **Run** any agent on the spot and watch the output appear
- Read the **outreach queue** and hit Send or Reject on each draft
- Scroll **recent runs**

Bookmark it on your phone. No curl needed.

**5. Fire Houston Scout.**

Hit **Run** next to Houston Scout. Give it 30–60 seconds — it's genuinely searching the web. You should get a Discord notification with scored Houston opportunities and rows in your Supabase `events` table.
...
|---|---|---|---|
| GET | `/` | — | status page |
| GET | `/admin` | — | the dashboard (its calls need the token) |
| GET | `/city` | — | the 3D yard (needs the token to load data) |
| GET | `/state` | admin | live agent state, JSON |
| GET | `/health` | — | is it up |
...
Auth means the header `x-admin-pin: YOUR_ADMIN_PIN`.
